                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            ...
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('organizers.storeRole', $data['id_organizer']) : route('organizers.updateRole', ['id_organizer' => base64_encode($data['organizer_role']->id_organizer), 'id_role' => base64_encode($data['organizer_role']->id)]) }}">
                                    ...
                                    <div class="form-group row">
                                        <label for="access_master" class="col-sm-2 col-form-label">Permissions</label>
                                        <div class="col-sm-10">
                                            <select class="form-control" id="permissions" required data-bind="selectedOptions: permissions, valueAllowUnset: true, options: $root.availableRolePermission, 
                                            optionsText: 'nama', optionsValue: 'id', select2: { placeholder: 'Choose Access Master', 
...
<script>
    ...
    function RoleViewModel() {
        ...
        self.availableRolePermission = ko.observableArray(<?php if (isset($data['role_permission'])) echo $data['role_permission'] ?>);

        self.nama = ko.observable('<?php if (isset($data['organizer_role'])) echo $data['organizer_role']->nama ?>');
        self.keterangan = ko.observable('<?php if (isset($data['organizer_role'])) echo $data['organizer_role']->keterangan ?>');
        self.permissions = ko.observableArray(<?php if (isset($data['organizer_role'])) echo $data['organizer_role']->permissions->pluck('id') ?>);
        self.status = ko.observable(<?= (isset($data['organizer_role'])) ? (($data['organizer_role']->status == 1) ? 'true' : 'false') : 'true' ?>);
    }
