                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Promo
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('promos.store') : route('promos.update', base64_encode($data['promo']->id)) }}">
                                    @if($data['actions']=='update') @method('PUT') @endif
                                    @csrf
                                    <div class="form-group row">
                                        <label for="nama" class="col-sm-2 col-form-label">Nama</label>
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="nama" name="nama" placeholder="Enter nama" data-bind="value: nama" required>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="keterangan" class="col-sm-2 col-form-label">Keterangan</label>
                                        <div class="col-sm-10">
                                            <textarea class="textarea form-control" rows="8" name="keterangan" data-bind="wysiwyg: keterangan"></textarea>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="discount" class="col-sm-2 col-form-label">Discount</label>
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="discount" name="discount" placeholder="Enter discount" data-bind="value: discount" required>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" id="qty" name="qty" placeholder="Enter qty" data-bind="value: qty" required>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="tanggal_awal" class="col-sm-2 col-form-label">Tanggal Awal</label>
                                        <div class="col-sm-10">
                                            <input type="date" name="tanggal_awal" data-bind="value: tanggal_awal">
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        <label for="tanggal_akhir" class="col-sm-2 col-form-label">Tanggal Akhir</label>
                                        <div class="col-sm-10">
                                            <input type="date" name="tanggal_akhir" data-bind="value: tanggal_akhir">
                                        </div>
                                    </div>
                                    <div class="form-group row">
...
<script>
    ...
    function PromoViewModel() {
        ...
        self.nama = ko.observable('<?php if (isset($data['promo'])) echo $data['promo']->nama ?>');
        self.keterangan = ko.observable('<?php if (isset($data['promo'])) echo $data['promo']->keterangan ?>');
        self.status = ko.observable(<?= (isset($data['promo'])) ? (($data['promo']->status == 1) ? 'true' : 'false') : 'true' ?>);
        self.discount = ko.observable('<?php if (isset($data['promo'])) echo $data['promo']->discount ?>');
        self.code = ko.observable('<?php if (isset($data['promo'])) echo $data['promo']->code ?>');
        self.qty = ko.observable('<?php if (isset($data['promo'])) echo $data['promo']->qty ?>');
        self.tanggal_awal = ko.observable('<?php if (isset($data['promo'])) echo $data['promo']->tanggal_awal ?>');
        self.tanggal_akhir = ko.observable('<?php if (isset($data['promo'])) echo $data['promo']->tanggal_akhir ?>');
    }

    ko.applyBindings(new PromoViewModel());
</script>
@endsection