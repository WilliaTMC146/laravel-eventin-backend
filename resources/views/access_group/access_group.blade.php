                            ...
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('access_groups.store') : route('access_groups.update', base64_encode($data['access_group']->id)) }}">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-10">
                                            <select class="form-control" id="access_masters" name="access_masters[]" required data-bind="valueAllowUnset: true, options: $root.availableAccessMaster, 
...
<script>
    ...
    function SubjectViewModel() {
        ...
        self.nama = ko.observable('<?php if (isset($data['access_group'])) echo $data['access_group']->nama ?>');
        self.keterangan = ko.observable('<?php if (isset($data['access_group'])) echo $data['access_group']->keterangan ?>');
    }

    ko.applyBindings(new SubjectViewModel());

    $(document).ready(function() {
        var access_masters = <?= (isset($data['access_group'])) ? $data['access_group']->access_masters : json_encode([]) ?>;
        $('#access_masters').val(access_masters.map(e => e.id)).change();
    });
</script>
@endsection