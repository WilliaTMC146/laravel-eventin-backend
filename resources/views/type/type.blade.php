                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Event Type
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('types.store') : route('types.update', base64_encode($data['type']->id)) }}">
...
<script>
    ...
    function TypeViewModel() {
        var self = this;

        self.nama = ko.observable('<?php if (isset($data['type'])) echo $data['type']->nama ?>');
        self.keterangan = ko.observable('<?php if (isset($data['type'])) echo $data['type']->keterangan ?>');
        self.status = ko.observable(<?= (isset($data['type'])) ? (($data['type']->status == 1) ? 'true' : 'false') : 'true' ?>);
    }

    ko.applyBindings(new TypeViewModel());
</script>
@endsection