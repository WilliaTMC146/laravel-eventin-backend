                    ...
                    <div class="col-md-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                else echo 'Ubah'; ?> Role Permission
                            </header>
                            <div class="panel-body" id="toro-area">
                                <form id="toro-form" method="POST" action="{{ ($data['actions'] == 'store') ? route('role_permissions.store') : route('role_permissions.update', base64_encode($data['role_permission']->id)) }}">
...
<script>
    ...
    function RolePermissionViewModel() {
        ...
        self.nama = ko.observable('<?php if (isset($data['role_permission'])) echo $data['role_permission']->nama ?>');
        self.keterangan = ko.observable('<?php if (isset($data['role_permission'])) echo $data['role_permission']->keterangan ?>');
        self.status = ko.observable(<?= (isset($data['role_permission'])) ? (($data['role_permission']->status == 1) ? 'true' : 'false') : 'true' ?>);
    }

    ko.applyBindings(new RolePermissionViewModel());
</script>
@endsection