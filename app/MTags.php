...
class MTags extends Model
{
    ...
    {
        return $this->belongsToMany('App\EventModel', 'event_tags', 'id_m_tags', 'id_event');
    }

    public function filter($order_field, $order_ascdesc, $search, $search_column, $limit, $startLimit)
    {
        $sql = MTags::select('m_tags.*', 'user_create.name as user_create_name', 'user_update.name as user_update_name')
        ->join('users as user_create', 'user_create.id', 'm_tags.created_id')
        ->join('users as user_update', 'user_update.id', 'm_tags.updated_id')
        ->orderBy($order_field, $order_ascdesc);

        if ($search != '' && $search != NULL) {
            $sql->where('m_tags.id', 'LIKE', "%{$search}%")
            ->orWhere('m_tags.nama', 'LIKE', "%{$search}%")
            ->orWhere('m_tags.keterangan', 'LIKE', "%{$search}%")
            ->orWhere('m_tags.status', 'LIKE', "%{$search}%")
            ->orWhere('m_tags.created_at', 'LIKE', "%{$search}%")
            ->orWhere('user_create.name', 'LIKE', "%{$search}%")
            ->orWhere('m_tags.updated_at', 'LIKE', "%{$search}%")
            ->orWhere('user_update.name', 'LIKE', "%{$search}%");
        }

        if ($search_column['id'] != '' && $search_column['id'] != NULL) {
            $sql->where('m_tags.id', 'LIKE', "%{$search_column['id']}%");
        }
        if ($search_column['nama'] != '' && $search_column['nama'] != NULL) {
            $sql->where('m_tags.nama', 'LIKE', "%{$search_column['nama']}%");
        }
        if ($search_column['keterangan'] != '' && $search_column['keterangan'] != NULL) {
            $sql->where('m_tags.keterangan', 'LIKE', "%{$search_column['keterangan']}%");
        }
        if ($search_column['status'] != '' && $search_column['status'] != NULL) {
            $sql->where('m_tags.status', 'LIKE', "%{$search_column['status']}%");
        }
        if ($search_column['created_at'] != '' && $search_column['created_at'] != NULL) {
            $sql->where('m_tags.created_at', 'LIKE', "%{$search_column['created_at']}%");
        }
        ...
        if ($search_column['updated_at'] != '' && $search_column['updated_at'] != NULL) {
            $sql->where('m_tags.updated_at', 'LIKE', "%{$search_column['updated_at']}%");
        }
