...
class MCategory extends Model
{
    ...
    {
        return $this->hasMany('App\EventModel', 'id_m_category', 'id');
    }

    public function filter($order_field, $order_ascdesc, $search, $search_column, $limit, $startLimit)
    {
        $sql = MCategory::select('m_category.*', 'user_create.name as user_create_name', 'user_update.name as user_update_name')
        ->join('users as user_create', 'user_create.id', 'm_category.created_id')
        ->join('users as user_update', 'user_update.id', 'm_category.updated_id')
        ->orderBy($order_field, $order_ascdesc);

        if ($search != '' && $search != NULL) {
            $sql->where('m_category.id', 'LIKE', "%{$search}%")
            ->orWhere('m_category.nama', 'LIKE', "%{$search}%")
            ->orWhere('m_category.keterangan', 'LIKE', "%{$search}%")
            ->orWhere('m_category.status', 'LIKE', "%{$search}%")
            ->orWhere('m_category.created_at', 'LIKE', "%{$search}%")
            ->orWhere('user_create.name', 'LIKE', "%{$search}%")
            ->orWhere('m_category.updated_at', 'LIKE', "%{$search}%")
            ->orWhere('user_update.name', 'LIKE', "%{$search}%");
        }

        if ($search_column['id'] != '' && $search_column['id'] != NULL) {
            $sql->where('m_category.id', 'LIKE', "%{$search_column['id']}%");
        }
        if ($search_column['nama'] != '' && $search_column['nama'] != NULL) {
            $sql->where('m_category.nama', 'LIKE', "%{$search_column['nama']}%");
        }
        if ($search_column['keterangan'] != '' && $search_column['keterangan'] != NULL) {
            $sql->where('m_category.keterangan', 'LIKE', "%{$search_column['keterangan']}%");
        }
        if ($search_column['status'] != '' && $search_column['status'] != NULL) {
            $sql->where('m_category.status', 'LIKE', "%{$search_column['status']}%");
        }
        if ($search_column['created_at'] != '' && $search_column['created_at'] != NULL) {
            $sql->where('m_category.created_at', 'LIKE', "%{$search_column['created_at']}%");
        }
        ...
        if ($search_column['updated_at'] != '' && $search_column['updated_at'] != NULL) {
            $sql->where('m_category.updated_at', 'LIKE', "%{$search_column['updated_at']}%");
        }
